...
void MutableVertexPartition::move_node(size_t v,size_t new_comm)
{
    ...
    for (size_t mode_i = 0; mode_i < 2; mode_i++)
    {
        ...
        vector< pair<size_t, size_t> >* neigh_edges = this->graph->get_neighbour_edges(v, mode);

        for (vector< pair<size_t, size_t> >::iterator v_it = neigh_edges->begin(); v_it != neigh_edges->end(); v_it++)
        {
            size_t u = v_it->first;
            size_t e = v_it->second;
